...
export default function ShapePreview({ item }) {
  ...
  return (
    <div className="rounded-lg p-2 bg-slate-900/60 border border-slate-700/50 backdrop-blur-sm shadow-lg" style={{ width: w * scale + 8, height: h * scale + 8 }}>
      <div className="relative" style={{ width: w * scale, height: h * scale }}>
        {blocks.map(([x, y], i) => (
          <div key={i} className={`absolute rounded-sm ${item.color} shadow-sm`} style={{
